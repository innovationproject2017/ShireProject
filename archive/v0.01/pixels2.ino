...
#define DEL 25                            // The delay (in ms) between each time step
...
uint16_t i = 0;
int tim = 0;                              // Current stage in the programming process (function dependent)
...
void sweep_left(uint16_t n, uint8_t x, uint8_t y){
    // Loop back when we reach the end of the sequence
    if (tim > SWEEP_LEFT_TIM_MAX){
        tim = -1;
    }
